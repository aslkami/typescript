...
let x: [string, number];  // 定义了数组元素只能是字符串或者浮点类型
x = ['hello', 10]; // OK
...
enum Color { Red = 1, Green, Blue = 6 } //  默认从0开始为元素编号。 你也可以手动的指定成员的数值。
...
declare function create(o: object | null): void;  // 这里 转化成 js 的时候无法识别
create({ prop: 0 }); // OK
create(null); // OK
// create(42); // Error
// create("string"); // Error
